...
pheno_current <-  read.csv('data/projected_bloomdates_ensemble_observed_weather.csv') %>% 
  mutate(species = tolower(species),
         ...
         pheno_predicted = replace_na(pheno_predicted, val_na),
...
master_pheno <- read.csv('data/master_phenology_repeated_splits.csv') %>% 
  ...
  na.omit() #get rid of the 1958 in klein altendorf, this season was incomplete
...
err2_df %>% 
  ...
  scale_fill_manual(values = c("#009E73", "#009E73", "#E69F00","#E69F00" ,"#E69F00", "#56B4E9", "#56B4E9" ),
                    breaks = c('Apple','Pear',  'Apricot', 'Europ. Plum', 'Sweet Cherry', 'Almond', 'Pistachio')) +
  facet_grid(~location, scales = 'free_x', space = 'free_x') +
  theme_bw(base_size = 15) +
  theme(axis.text.x = element_text(angle = 45, hjust=1), legend.position = 'none')

ggsave('figures/paper/error_types_two.jpeg', height = 20, width = 30, units = 'cm', device = 'jpeg')
...
thermal_time_window_2020 <- purrr::map(c('begin_flowering_f5', 'flowering_f50'), function(x) LarsChill::get_thermal_window_phenology(weather_list_obs = weather_list_obs, 
  ...
  bind_rows() %>% 
  mutate(species = tolower(species))
write.csv(thermal_time_window_2020, file = 'data/timewindow_hist.csv', row.names = FALSE)
...
future_timewindow_df <- data.frame()
...
for(ssp in unique(ssp_vec)){
  ...
  for(gcm in unique(gcm_vec)){
    ...
    for(time in unique(time_vec)){
      ...
      future_timewindow_df <- rbind(future_timewindow_df, thermal_time_window)
