...
thermal_window %>% 
  ...
  facet_grid(flowering_stage~species)
...
window_df <- thermal_window %>% 
  mutate(flowering_type = recode(flowering_stage, f5 = 'begin_flowering_f5', f50 = 'flowering_f50'),
         lower =  min_doy,
         upper = max_doy,
...
for(scen in names(fut_weather_list)){
  ...
  for(yr in names(fut_weather_list[[scen]])){
    ...
    thermal_window <- get_thermal_window_phenology(weather_list_obs = weather_list_obs,
                                                   weather_list_pred = fut_weather_list[[scen]][[yr]], 
                                                   observation_df = observation_df, 
                                                   frost_threshold = frost_threshold, 
                                                   heat_threshold = heat_threshold, 
                                                   target_col_obs = target_col_obs)
