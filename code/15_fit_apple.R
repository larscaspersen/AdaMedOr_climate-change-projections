...
#make hourly
lat_cka <- stations %>% 
  filter(station_name == 'Klein-Altendorf') %>% 
  pull(latitude)

lat_meknes <- stations %>% 
  filter(station_name == 'Meknes') %>% 
  pull(latitude)


cka_hourly <- stack_hourly_temps(cka, latitude = lat_cka)
meknes_hourly <- stack_hourly_temps(meknes, latitude = lat_meknes)
...
SeasonList <- list('Meknes' = meknes_season, 
...
dir.create('data/fitting/apple')
source('code/utilities/load_save_fitting_results.R')

#save data
for(i in 1:10){
  save_fitting_list(res_list3[[i]], path = 'data/fitting/apple/', prefix = paste0('repeat', i, '_'))
}
